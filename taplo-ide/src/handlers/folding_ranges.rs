                    ...
                    header_starts.retain(|(k, h)| {
                        if k == &key || !key.starts_with(k) {
                            folding_ranges.push(FoldingRange {
                                ...
                                end_line: mapper
                                    .position(
                                        e.end().checked_sub(TextSize::from(1)).unwrap_or(e.end()),
                                    )
                                    .unwrap()
                                    .line,
                                end_character: None,
                            ...
                            match d.kind() {
                                ARRAY => {
                                    if d.descendants_with_tokens().any(|t| t.kind() == NEWLINE) {
                                        ...
                                        let end = mapper.position(d.text_range().end()).unwrap();
    ...
    if let Some(e) = &last_non_header {
        for (_, h) in header_starts {
            folding_ranges.push(FoldingRange {
                ...
                start_character: None,
                end_line: mapper
                    .position(e.end().checked_sub(TextSize::from(1)).unwrap_or(e.end()))
                    .unwrap()
                    .line,
                end_character: None,
                kind: Some(FoldingRangeKind::Region),
